...
- Navigate画面の下部へ自動消灯メニューを追加し、0〜59分のスライダーと0〜24時間のスライダーで本体側の無操作タイマーを設定できるようにした。0分0時間は本体側の自動消灯を無効にする。設定はNVSへ保存する。
- Navigate画面をスクロール対応にし、内容ドラッグで移動、スライダーはドラッグで追従するようにした。
- 不具合報告、開発参加、脆弱性報告、議論時の扱いを分かりやすくするため、サポート・貢献・セキュリティ・行動規範の文書を追加した。
