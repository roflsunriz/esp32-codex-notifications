...
PCがスリープまたはシャットダウンするとDesktopからAuto-dim RPCを受信できないため、ESP32側でBLE切断から30秒の固定フォールバックタイマーを開始します。期限前に再接続した場合はタイマーをキャンセルし、期限時点でも未接続ならAuto-dimと同じDisplay Off、Sleep In、バックライトLOWへ移行します。電源投入時に一度も接続できなかった場合も、BLE advertising開始から30秒後に消灯します。

切断中の最初のタッチは接続中と同様に復帰専用です。ホストへHID通知は送信できませんが、ローカル表示を復帰して切断タイマーを30秒へ戻します。再接続時は期限の前後にかかわらず画面をONにし、接続直後のAuto-dim復帰猶予も再設定します。期限判定は符号付き差分で行い、ESP32の `millis()` が周回しても30秒を維持します。
